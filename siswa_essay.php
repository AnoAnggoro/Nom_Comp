...
if ($action === 'submit_answer') {
    ...
    if ($taskId <= 0) {
        $errors[] = 'Tugas tidak valid.';
    }

    if ($answerText === '') {
        $errors[] = 'Jawaban essay tidak boleh kosong.';
    }
    ...
    if (!$taskRow) {
        $errors[] = 'Tugas essay tidak ditemukan.';
    } elseif (!empty($taskRow['due_at']) && strtotime((string) $taskRow['due_at']) !== false && time() > strtotime((string) $taskRow['due_at'])) {
        $errors[] = 'Deadline tugas sudah lewat, jawaban tidak bisa dikumpulkan.';
    }

    if (count($errors) === 0) {
        $check = $pdo->prepare('SELECT id, graded_at FROM essay_answers WHERE task_id = :task_id AND student_id = :student_id LIMIT 1');
        $check->execute(['task_id' => $taskId, 'student_id' => $user['id']]);
        $existing = $check->fetch();

        if ($existing && !empty($existing['graded_at'])) {
            $errors[] = 'Jawaban sudah dinilai guru dan tidak bisa diubah lagi.';
        } else {
            if ($existing) {
                $update = $pdo->prepare(
                    'UPDATE essay_answers
                     SET answer_text = :answer_text, submitted_at = NOW()
                     WHERE id = :id'
                );
                $update->execute([
                ...
                $insert = $pdo->prepare(
                    'INSERT INTO essay_answers (task_id, student_id, answer_text)
                     VALUES (:task_id, :student_id, :answer_text)'
                );
                $insert->execute([
                    'task_id' => $taskId,
                    'student_id' => $user['id'],
            ...
            chemnama_flash('Jawaban essay berhasil disimpan.', 'success');
            header('Location: siswa_essay.php');
            exit;
        }
    }
}
...
$essayTaskStmt = $pdo->prepare(
    'SELECT t.*, m.badge AS module_badge, m.title AS module_title,
            a.id AS answer_id, a.answer_text, a.score, a.feedback, a.submitted_at, a.graded_at
     FROM essay_tasks t
     JOIN modules m ON m.id = t.module_id
     JOIN users g ON g.id = t.created_by
     LEFT JOIN user_profiles up_teacher ON up_teacher.user_id = g.id
     LEFT JOIN essay_answers a ON a.task_id = t.id AND a.student_id = :student_id
...
$essayTaskStmt->execute([
    'student_id' => $user['id'],
    'kelas_exact' => $studentClass,
    'kelas_fallback' => $studentClass,
]);
$essayTasks = $essayTaskStmt->fetchAll();

$essayCountStmt = $pdo->prepare(
...
$essayCountStmt->execute([
    'kelas_exact' => $studentClass,
    'kelas_fallback' => $studentClass,
]);
$essayCountRows = $essayCountStmt->fetchAll();

$homeworkTaskStmt = $pdo->prepare(
    'SELECT t.*, m.badge AS module_badge, m.title AS module_title,
...
$homeworkTaskStmt->execute([
    'student_class_count' => $studentClass,
    'student_id' => $user['id'],
    'kelas_exact' => $studentClass,
    'kelas_fallback' => $studentClass,
]);
$homeworkTasks = $homeworkTaskStmt->fetchAll();
...
$homeworkCountRows = $homeworkCountStmt->fetchAll();

$moduleCounts = [];
$essayTotalCount = 0;
$homeworkTotalCount = 0;
foreach ($essayCountRows as $row) {
    $mid = (int) $row['module_id'];
    $cnt = (int) $row['total'];
    $moduleCounts[$mid] = ($moduleCounts[$mid] ?? 0) + $cnt;
    $essayTotalCount += $cnt;
}
foreach ($homeworkCountRows as $row) {
    $mid = (int) $row['module_id'];
    $cnt = (int) $row['total'];
    $moduleCounts[$mid] = ($moduleCounts[$mid] ?? 0) + $cnt;
    $homeworkTotalCount += $cnt;
}

$totalCount = $essayTotalCount + $homeworkTotalCount;
...
<head>
    ...
    <title>Exercise Siswa - Nom Comp</title>
...
<main class="guru-page student-page">
    ...
    <aside class="guru-sidebar student-sidebar" id="studentSidebar">
        ...
        <div class="guru-menu-block">
            ...
            <nav class="guru-menu-list">
                ...
                <a class="guru-menu-item is-active" href="siswa_essay.php"><?= chemnama_icon('edit', '#0f9d58'); ?><span>Exercise</span></a>
    ...
    <section class="guru-content student-content">
        ...
        <div class="siswa-essay-head">
            <h1>Exercise</h1>
            <p>Jawab tugas essay dan pr dari guru, lalu tunggu proses penilaian.</p>
        </div>
        ...
        <div class="essay-tabs" data-filter-tabs="essay">
            <a class="essay-tab js-module-tab <?= $selectedModule === 0 ? 'is-active' : ''; ?>" href="siswa_essay.php" data-module="0">Semua (<?= $totalCount; ?>)</a>
            <?php foreach ($modules as $module): ?>
                <?php $count = $moduleCounts[(int) $module['id']] ?? 0; ?>
                <a class="essay-tab js-module-tab <?= $selectedModule === (int) $module['id'] ? 'is-active' : ''; ?>" href="siswa_essay.php?module=<?= chemnama_e((string) $module['id']); ?>" data-module="<?= chemnama_e((string) $module['id']); ?>">
        ...
        <div class="module-filter-mobile" data-filter-mobile="essay">
            ...
            <div class="module-filter-dropdown" data-filter-dropdown="essay">
                <button class="module-filter-trigger" type="button" data-filter-trigger="essay" aria-expanded="false" aria-controls="siswaEssayFilterMenu">
                ...
                <div class="module-filter-menu" id="siswaEssayFilterMenu" data-filter-menu="essay" hidden>
        ...
        <div class="essay-list" data-filter-list="essay">
            <article class="panel-card empty-state" data-filter-empty="essay" style="<?= count($essayTasks) === 0 ? '' : 'display:none;'; ?>">
                <h3>Belum ada tugas essay</h3>
                <p>Tugas dari guru akan tampil di sini.</p>
            </article>

            <?php foreach ($essayTasks as $task): ?>
                <?php
                    $hasAnswer = !empty($task['answer_id']);
                    $isGraded = !empty($task['graded_at']);
                ?>
                <article class="panel-card siswa-essay-item" data-module-id="<?= chemnama_e((string) $task['module_id']); ?>">
                    <div class="essay-item-head">
                        ...
                        <span class="essay-status <?= $isGraded ? 'graded' : ($hasAnswer ? 'submitted' : 'pending'); ?>">
                            <?= $isGraded ? 'Dinilai' : ($hasAnswer ? 'Dikumpulkan' : 'Belum dijawab'); ?>
                        </span>
                    </div>

                    <p class="essay-question"><?= nl2br(chemnama_e((string) $task['prompt_text'])); ?></p>

                    <form method="post" class="essay-answer-form">
                        <input type="hidden" name="action" value="submit_answer">
                        <input type="hidden" name="task_id" value="<?= chemnama_e((string) $task['id']); ?>">

                        <label>
                            <span>Jawaban kamu</span>
                            <textarea name="answer_text" placeholder="Tulis jawaban essay di sini..." <?= $isGraded ? 'readonly' : ''; ?>><?= chemnama_e((string) ($task['answer_text'] ?? '')); ?></textarea>
                        </label>

                        <div class="essay-answer-meta">
                            <small>Deadline: <?= $task['due_at'] ? chemnama_e((string) $task['due_at']) : '-'; ?></small>
                            <small>Terakhir dikumpulkan: <?= $task['submitted_at'] ? chemnama_e((string) $task['submitted_at']) : '-'; ?></small>
                        </div>

                        <div class="form-action-row">
                            <button type="submit" class="btn btn-primary" <?= $isGraded ? 'disabled' : ''; ?>><?= $hasAnswer ? 'Perbarui Jawaban' : 'Kumpulkan Jawaban'; ?></button>
                        </div>
                    </form>

                    <?php if ($isGraded): ?>
                        <div class="essay-grade-result">
                            <strong>Nilai: <?= chemnama_e((string) ($task['score'] ?? '-')); ?></strong>
                            <?php if (!empty($task['feedback'])): ?>
                                <p><?= chemnama_e((string) $task['feedback']); ?></p>
                            <?php else: ?>
                                <p>Tidak ada catatan tambahan dari guru.</p>
                            <?php endif; ?>
                        </div>
                    <?php endif; ?>
                </article>
        ...
        <div class="essay-section-divider" id="pr-homework">
            <span>PR / Homework</span>
        </div>

        <div class="homework-list" data-filter-list="homework">
            <article class="panel-card empty-state" data-filter-empty="homework" style="<?= count($homeworkTasks) === 0 ? '' : 'display:none;'; ?>">
                <h3>Belum ada PR</h3>
                <p>PR dari guru akan tampil di sini.</p>
            </article>

            <?php foreach ($homeworkTasks as $task): ?>
                <?php
                    $submitted = !empty($task['submission_id']);
                    $checked = (int) ($task['is_checked'] ?? 0) === 1;
                ?>
                <article class="panel-card siswa-homework-item" data-module-id="<?= chemnama_e((string) $task['module_id']); ?>">
                    <div class="essay-item-head">
                        <div>
...
<script>
    (function () {
        ...
        const essayItems = document.querySelectorAll('[data-filter-list="essay"] .siswa-essay-item');
        const homeworkItems = document.querySelectorAll('[data-filter-list="homework"] .siswa-homework-item');
        const essayEmptyState = document.querySelector('[data-filter-empty="essay"]');
        const homeworkEmptyState = document.querySelector('[data-filter-empty="homework"]');

        if (!tabs.length || !essayEmptyState || !homeworkEmptyState) {
            return;
        }

        const initialModuleMatch = window.location.search.match(/[?&]module=([^&]+)/);
        const initialModule = initialModuleMatch ? decodeURIComponent(initialModuleMatch[1]) : '0';

        function updateModuleInUrl(moduleId) {
            const baseUrl = window.location.pathname;
            const queryParts = window.location.search.replace(/^\?/, '').split('&').filter(Boolean).filter((part) => !part.startsWith('module='));

            if (moduleId !== '0') {
                queryParts.push('module=' + encodeURIComponent(moduleId));
            }

            const nextUrl = queryParts.length > 0 ? baseUrl + '?' + queryParts.join('&') : baseUrl;
            window.history.replaceState({}, '', nextUrl + window.location.hash);
        }

        function applyFilter(moduleId, updateUrl) {
            let essayVisibleCount = 0;
            let homeworkVisibleCount = 0;
            ...
            homeworkItems.forEach((item) => {
                ...
                item.style.display = matches ? '' : 'none';
                if (matches) {
                    homeworkVisibleCount += 1;
                }
            });

            essayEmptyState.style.display = essayVisibleCount === 0 ? '' : 'none';
            homeworkEmptyState.style.display = homeworkVisibleCount === 0 ? '' : 'none';
